...
/**
 * Resolve the current provider-level tariff window without choosing a model.
 * This is display metadata only: every active scheduled rule must agree on the
 * same schedule, otherwise the result fails closed to null.
 */
export function resolveCurrentTariff({ identity, timestamp = Date.now(), currency = "USD" }, rules = PRICING_RULES) {
	ensureValidatedRules(rules);
	const at = timestampOf(timestamp);
	if (at === null || typeof currency !== "string") return null;
	const matches = rules.filter((rule) => (
		isEligibleIdentity(identity, rule)
		&& rule.currency === currency
		&& rule.schedule !== null
	...
	const scheduleKey = JSON.stringify(matches[0].schedule);
	if (!matches.every((rule) => JSON.stringify(rule.schedule) === scheduleKey)) return null;
	const schedule = matches[0].schedule;
	const tariff = tariffForSchedule(schedule, at);
	const firstMinute = Math.floor(at / 60000) * 60000 + 60000;
	const limit = firstMinute + 8 * 24 * 60 * 60000;
	let nextTransitionAt = null;
	for (let candidate = firstMinute; candidate <= limit; candidate += 60000) {
		if (tariffForSchedule(schedule, candidate) !== tariff) {
			nextTransitionAt = new Date(candidate).toISOString();
			break;
		}
	}
	return {
