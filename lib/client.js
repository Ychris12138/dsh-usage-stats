...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".usg_sessionPill{box-sizing:border-box;min-width:0;max-width:220px;height:28px;color:var(--dsw-alias-label-tertiary);background:var(--dsw-alias-fill-l1,transparent);border:1px solid var(--dsw-alias-border-l2);border-radius:999px;align-items:center;gap:5px;padding:0 8px;font:inherit;font-size:11px;line-height:16px;cursor:pointer;display:inline-flex;flex:0 1 auto;white-space:nowrap;overflow:hidden}",
			...
			".usg_sessionPillProvider{min-width:0;max-width:88px;color:var(--dsw-alias-label-primary);font-weight:500;text-overflow:ellipsis;overflow:hidden}",
			".usg_sessionPillSeparator{color:var(--dsw-alias-label-caption);flex:none}",
			".usg_sessionPillValue{min-width:0;max-width:112px;font-variant-numeric:tabular-nums;text-overflow:ellipsis;overflow:hidden}",
		...
		function providerChoiceLabel(provider, translate) {
			const label = typeof provider?.displayName === "string" && provider.displayName.trim() !== ""
				...
				: String(provider?.id ?? "");
			if (provider?.id !== "orcarouter") return label;
