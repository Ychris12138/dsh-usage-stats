...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".usg_orcaCard{border:1px solid color-mix(in srgb,#0891b2 32%,var(--dsw-alias-border-l2));background:linear-gradient(135deg,color-mix(in srgb,#0891b2 10%,transparent),transparent 55%);border-radius:12px;margin-top:10px;padding:9px 11px;display:flex;flex-wrap:wrap;align-items:center;gap:5px 8px}",
			".usg_orcaTitle{color:var(--dsw-alias-label-primary);font-size:12px;font-weight:600;line-height:18px}",
			".usg_orcaSponsor{color:var(--dsw-alias-label-tertiary);font-size:10px;line-height:16px}",
			".usg_orcaDescription{color:var(--dsw-alias-label-secondary);width:100%;margin:0;font-size:11px;line-height:16px}",
			".usg_orcaLink{color:#0891b2;font-size:11px;line-height:24px;text-decoration:none}",
			".usg_orcaLink:hover{text-decoration:underline}",
			".usg_orcaButton{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-fill-l1,transparent);border:1px solid var(--dsw-alias-border-l2);border-radius:7px;margin-left:auto;padding:3px 7px;font:inherit;font-size:11px;line-height:18px;cursor:pointer}",
			".usg_orcaButton:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover)}",
			".usg_orcaButton:disabled{color:var(--dsw-alias-label-tertiary);cursor:default}",
			".usg_orcaDisclosure{color:var(--dsw-alias-label-caption);width:100%;margin:0;font-size:9px;line-height:14px}",
			".usg_orcaError{color:var(--dsw-alias-state-error-primary);width:100%;margin:0;font-size:10px;line-height:15px}",
		...
		const S = {
			...
			orcaCard: "usg_orcaCard",
			orcaTitle: "usg_orcaTitle",
			orcaSponsor: "usg_orcaSponsor",
			orcaDescription: "usg_orcaDescription",
			orcaLink: "usg_orcaLink",
			orcaButton: "usg_orcaButton",
			orcaDisclosure: "usg_orcaDisclosure",
			orcaError: "usg_orcaError",
		...
		const ORCAROUTER_REFERRAL_URL = "https://www.orcarouter.ai/ref/ref_13c34663d1527ac16963";
		...
		async function addOrcaRouterIntegration(request = fetchJson) {
			...
			if (payload?.ok !== true || payload.integration === null || typeof payload.integration !== "object") throw new Error("provider preset was not added");
			return payload.integration;
		}

		/** Compact sponsored preset card; installation remains an explicit click. */
		function OrcaRouterIntegrationCard({ integration, pending, error, translate, onAdd }) {
			const installed = integration?.installed === true;
			const available = integration?.available === true;
			return react_jsx_runtime.jsxs("article", {
				className: S.orcaCard,
				"data-orcarouter-integration": true,
				"data-installed": installed,
				children: [
					react_jsx_runtime.jsx("span", { className: S.orcaTitle, children: `🐋 ${translate("orcarouter.title")}` }),
					react_jsx_runtime.jsx("span", { className: S.orcaSponsor, children: translate("orcarouter.sponsored") }),
					react_jsx_runtime.jsx("p", { className: S.orcaDescription, children: translate("orcarouter.description") }),
					react_jsx_runtime.jsx("a", {
						className: S.orcaLink,
						href: ORCAROUTER_REFERRAL_URL,
						target: "_blank",
						rel: "sponsored noopener noreferrer",
						children: translate("orcarouter.try")
					}),
					react_jsx_runtime.jsx("button", {
						type: "button",
						className: S.orcaButton,
						"data-orcarouter-add": true,
						disabled: installed || !available || pending,
						onClick: onAdd,
						children: installed ? translate("orcarouter.added")
							: pending ? translate("orcarouter.adding")
								: available ? translate("orcarouter.add") : translate("orcarouter.unavailable")
					}),
					error !== null ? react_jsx_runtime.jsx("p", { className: S.orcaError, children: translate("orcarouter.error") }) : null,
					react_jsx_runtime.jsx("p", { className: S.orcaDisclosure, children: translate("orcarouter.disclosure") })
				]
			});
		}
		...
		function UsageStatsPanel({ wide, t }) {
			...
			const [orcaRouterError, setOrcaRouterError] = react.useState(null);
			...
			const loadOrcaRouter = react.useCallback(() => {
				setOrcaRouterError(null);
				loadOrcaRouterIntegration().then((integration) => {
					if (mountedRef.current) setOrcaRouterIntegration(integration);
				}).catch(() => {
					if (mountedRef.current) setOrcaRouterError("unavailable");
				});
			}, []);

			const addOrcaRouter = react.useCallback(() => {
				setOrcaRouterPending(true);
				setOrcaRouterError(null);
				addOrcaRouterIntegration().then((integration) => {
					if (!mountedRef.current) return;
					setOrcaRouterIntegration(integration);
					loadProviders();
				}).catch(() => {
					if (mountedRef.current) setOrcaRouterError("rejected");
				}).finally(() => {
			...
			}, [open, loadUsage, loadProviders, loadOrcaRouter]);
						...
						children: [
							...
							react_jsx_runtime.jsxs("div", {
								...
								onSelect: selectProvider,
											...
											react_jsx_runtime.jsx(OrcaRouterIntegrationCard, {
												integration: orcaRouterIntegration,
												pending: orcaRouterPending,
												error: orcaRouterError,
												translate,
												onAdd: addOrcaRouter
											}),
											react_jsx_runtime.jsx("section", {
		...
		const zh = {
			...
			"orcarouter.sponsored": "赞助集成",
			"orcarouter.description": "可选的 OpenAI-compatible Provider，内置 orcarouter/auto；API Key 由你在 DSH 中配置。",
			"orcarouter.try": "了解 OrcaRouter",
			"orcarouter.add": "添加到模型",
			"orcarouter.adding": "正在添加…",
			"orcarouter.added": "已添加",
			"orcarouter.unavailable": "当前 DSH 不支持",
			"orcarouter.error": "未能更新 DSH Provider 设置，请稍后重试。",
			"orcarouter.disclosure": "OrcaRouter 赞助本项目；此邀请链接的合格注册可能为维护者带来推荐收益。",
			"account.title": "供应商账户",
		...
		const en = {
			...
			"orcarouter.title": "OrcaRouter",
			"orcarouter.sponsored": "Sponsored integration",
			"orcarouter.description": "Optional OpenAI-compatible provider with orcarouter/auto. You configure the API key in DSH.",
			"orcarouter.try": "Try OrcaRouter",
			"orcarouter.add": "Add to Models",
			"orcarouter.adding": "Adding…",
			"orcarouter.added": "Added",
			"orcarouter.unavailable": "Unavailable in this DSH",
			"orcarouter.error": "DSH rejected the provider settings update. Please retry.",
			"orcarouter.disclosure": "OrcaRouter sponsors this project. Qualifying sign-ups through this referral link may benefit the maintainer.",
		...
		exports.OrcaRouterIntegrationCard = OrcaRouterIntegrationCard;
