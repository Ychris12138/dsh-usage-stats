	...
	return new Promise((resolve, reject) => {
		...
		const request = transport(url, {
			...
			lookup: (_hostname, options, callback) => {
				// Defer by one tick. A synchronous connect-phase failure (for example,
				// a transient EHOSTUNREACH) can otherwise be reported on the socket
				// before the request emits `socket` and installs its error handling,
				// leaving an unhandled TLSSocket error to terminate the dsh host (#111).
				process.nextTick(() => {
