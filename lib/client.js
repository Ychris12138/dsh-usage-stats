...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".usg_pricingWindow{color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-fill-l1,transparent);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;margin:-2px 0 8px;padding:5px 8px;font-size:11px;line-height:16px;font-variant-numeric:tabular-nums}",
			".usg_pricingWindow[data-tariff=peak]{color:var(--dsw-alias-state-warning-primary,var(--dsw-alias-label-secondary))}",
			".usg_pricingWindow[data-tariff=offPeak]{color:var(--dsw-alias-state-success-primary,var(--dsw-alias-label-secondary))}",
			...
			".usg_sessionCosts{flex-direction:column;display:flex}",
			".usg_sessionCostRow{align-items:center;gap:10px;border-bottom:1px solid var(--dsw-alias-border-l1);padding:6px 0;display:flex}",
			".usg_sessionCostRow:last-child{border-bottom:0}",
			".usg_sessionCostMain{min-width:0;flex:1;display:flex;flex-direction:column}",
			".usg_sessionCostTitle{color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;font-size:12px;line-height:18px;overflow:hidden}",
			".usg_sessionCostMeta{color:var(--dsw-alias-label-caption);text-overflow:ellipsis;white-space:nowrap;font-size:10px;line-height:15px;overflow:hidden}",
			".usg_sessionCostAmount{color:var(--dsw-alias-label-primary);font-size:12px;font-weight:600;line-height:18px;font-variant-numeric:tabular-nums;white-space:nowrap}",
		...
		const S = {
			...
			pricingWindow: "usg_pricingWindow",
			...
			dayHit: "usg_dayHit",
			dayBar: "usg_dayBar",
			sessionCosts: "usg_sessionCosts",
			sessionCostRow: "usg_sessionCostRow",
			sessionCostMain: "usg_sessionCostMain",
			sessionCostTitle: "usg_sessionCostTitle",
			sessionCostMeta: "usg_sessionCostMeta",
			sessionCostAmount: "usg_sessionCostAmount",
		...
		function buildSessionCostRows(sessions, limit = 20) {
			if (!Array.isArray(sessions) || !Number.isInteger(limit) || limit <= 0) return [];
			return sessions
				.filter((session) => session !== null && typeof session === "object"
					&& session.costComplete === true
					&& typeof session.estimatedCost === "number" && Number.isFinite(session.estimatedCost)
					&& typeof session.currency === "string" && session.currency !== "")
				.sort((a, b) => {
					const aTime = typeof a.lastAt === "string" ? Date.parse(a.lastAt) : Number.NaN;
					const bTime = typeof b.lastAt === "string" ? Date.parse(b.lastAt) : Number.NaN;
		...
		function sessionCostMeta(session, translate) {
			const tokens = typeof session?.tokens === "number" && Number.isFinite(session.tokens) ? fmt(session.tokens) : null;
			const models = Array.isArray(session?.models) ? session.models.filter((model) => typeof model === "string" && model !== "").slice(0, 2) : [];
			if (tokens !== null && models.length > 0) return translate("sessionCost.meta", { tokens, models: models.join(", ") });
			if (tokens !== null) return translate("sessionCost.tokensOnly", { tokens });
			return models.join(", ");
		}
		...
		function UsageStatsPanel({ wide, t }) {
			...
			react.useEffect(() => {
				if (!open) return void 0;
				const next = Date.parse(selectedProviderInfo?.pricingWindow?.nextTransitionAt ?? "");
				if (!Number.isFinite(next)) return void 0;
				const delay = Math.max(0, next - Date.now()) + 1000;
				const timer = window.setTimeout(loadProviders, Math.min(delay, 2_147_000_000));
				return () => window.clearTimeout(timer);
			}, [open, loadProviders, selectedProviderInfo?.pricingWindow?.nextTransitionAt]);
			...
			const sessionCostRows = react.useMemo(() => buildSessionCostRows(usage?.sessions), [usage]);
									...
									selectedEntry !== null ? react_jsx_runtime.jsx(DayDetail, {
										...
										children: [
											...
											react_jsx_runtime.jsx(ProviderPicker, {
												...
												tariffWindowText(selectedProviderInfo?.pricingWindow, Date.now(), translate) !== "" ? react_jsx_runtime.jsx("p", {
													className: S.pricingWindow,
													"data-tariff": selectedProviderInfo?.pricingWindow?.tariff ?? "",
													children: tariffWindowText(selectedProviderInfo?.pricingWindow, Date.now(), translate)
												}) : null,
										...
										sessionCostRows.length > 0 && react_jsx_runtime.jsxs("section", {
											className: S.section,
											children: [
												react_jsx_runtime.jsx("h3", { className: S.sectionTitle, children: translate("sessionCost.title") }),
												react_jsx_runtime.jsx("div", {
													className: S.sessionCosts,
													children: sessionCostRows.map((session) => react_jsx_runtime.jsxs("div", {
														className: S.sessionCostRow,
														"data-session-cost": session.sessionId ?? "",
														children: [
															react_jsx_runtime.jsxs("div", { className: S.sessionCostMain, children: [
																react_jsx_runtime.jsx("span", { className: S.sessionCostTitle, title: session.title ?? session.sessionId ?? "", children: session.title ?? session.sessionId ?? translate("sessionCost.untitled") }),
																sessionCostMeta(session, translate) !== "" ? react_jsx_runtime.jsx("span", { className: S.sessionCostMeta, children: sessionCostMeta(session, translate) }) : null
															] }),
															react_jsx_runtime.jsx("span", { className: S.sessionCostAmount, children: fmtCurrency(session.estimatedCost, session.currency) })
														]
													}, session.sessionId ?? `${session.title ?? "session"}-${session.lastAt ?? ""}`))
												})
											]
										}),
		...
		function tariffWindowText(window, now, translate) {
			if (window === null || window === void 0 || typeof window !== "object") return "";
			if (window.tariff !== "peak" && window.tariff !== "offPeak") return "";
			const baseKey = window.tariff === "peak" ? "pricing.peak" : "pricing.offPeak";
			if (typeof window.nextTransitionAt !== "string") return translate(baseKey);
			const next = Date.parse(window.nextTransitionAt);
			if (!Number.isFinite(next) || next <= now) return "";
			const totalMinutes = Math.max(1, Math.ceil((next - now) / 60000));
			const time = durationLabel(totalMinutes, translate);
			return translate(window.tariff === "peak" ? "pricing.peakCountdown" : "pricing.offPeakCountdown", { time });
		}
		...
		const zh = {
			...
			"pricing.peak": "峰时",
			"pricing.offPeak": "闲时",
			"pricing.peakCountdown": "峰时 · {time} 后进入闲时",
			"pricing.offPeakCountdown": "闲时 · {time} 后进入峰时",
			...
			"sessionCost.title": "最近会话费用",
			"sessionCost.untitled": "未命名会话",
			"sessionCost.meta": "{tokens} Token · {models}",
			"sessionCost.tokensOnly": "{tokens} Token",
			"export.title": "安全导出",
		...
		const en = {
			...
			"pricing.peakCountdown": "Peak · off-peak in {time}",
			"pricing.offPeakCountdown": "Off-peak · peak in {time}",
			...
			"sessionCost.title": "Recent session costs",
			"sessionCost.untitled": "Untitled session",
			"sessionCost.meta": "{tokens} tokens · {models}",
			"sessionCost.tokensOnly": "{tokens} tokens",
		...
		exports.buildSessionCostRows = buildSessionCostRows;
		exports.sessionCostMeta = sessionCostMeta;
		...
		exports.tariffWindowText = tariffWindowText;
