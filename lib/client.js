...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".usg_sessionInline{box-sizing:border-box;color:var(--dsw-alias-label-tertiary);background:none;border:0;border-radius:0;align-items:center;gap:0;padding:0;margin:0;font:inherit;font-size:11px;line-height:18px;cursor:pointer;display:inline-flex;flex:0 1 auto;min-width:0;max-width:clamp(56px,18vw,170px);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}",
			".usg_sessionInline:hover{text-decoration:underline;text-underline-offset:2px}",
			".usg_sessionInline:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:2px}",
			".usg_sessionInline[data-tone=normal]{color:var(--dsw-alias-label-secondary)}",
			".usg_sessionInline[data-tone=warning]{color:var(--dsw-alias-state-warn-label,var(--dsw-alias-state-warn-primary))}",
			".usg_sessionInline[data-tone=critical]{color:var(--dsw-alias-state-error-primary)}",
			".usg_sessionInlineProvider{min-width:0;color:var(--dsw-alias-label-primary);font-weight:500;white-space:nowrap;text-overflow:ellipsis;overflow:hidden;flex:1 1 auto}",
			".usg_sessionInlineSeparator{color:var(--dsw-alias-label-caption);flex:none}",
			".usg_sessionInlineAccount{min-width:0;font-variant-numeric:tabular-nums;white-space:nowrap;text-overflow:ellipsis;overflow:hidden;flex:0 1 auto}",
			".usg_sessionInlineCost{font-variant-numeric:tabular-nums;white-space:nowrap;flex:none}",
			"@media(max-width:540px){.usg_sessionInlineProvider,.usg_sessionInlineProviderSeparator{display:none}}",
			"@media(max-width:480px){.usg_sessionInline{display:none}}",
		...
		const S = {
			...
			sessionInline: "usg_sessionInline",
			sessionInlineProvider: "usg_sessionInlineProvider",
			sessionInlineSeparator: "usg_sessionInlineSeparator",
			sessionInlineProviderSeparator: "usg_sessionInlineProviderSeparator",
			sessionInlineAccount: "usg_sessionInlineAccount",
			sessionInlineCost: "usg_sessionInlineCost",
			panel: "usg_panel",
			header: "usg_header",
		...
		/** Pure zero-chrome inline view, separated for render/click regression tests. */
		function CurrentSessionInlineView({ snapshot, translate, onOpen = requestUsageStatsPanel, now = Date.now() }) {
			const view = sessionPillViewOf(snapshot, translate, now);
			if (view === null) return null;
			return react_jsx_runtime.jsxs("button", {
				type: "button",
				className: S.sessionInline,
				"data-current-session-inline": true,
				...
				children: [
					react_jsx_runtime.jsx("span", { className: S.sessionInlineProvider, children: view.providerLabel }),
					react_jsx_runtime.jsx("span", { className: `${S.sessionInlineSeparator} ${S.sessionInlineProviderSeparator}`, "aria-hidden": true, children: " · " }),
					react_jsx_runtime.jsx("span", { className: S.sessionInlineAccount, children: view.accountLabel }),
					react_jsx_runtime.jsx("span", { className: S.sessionInlineSeparator, "aria-hidden": true, children: " · " }),
					react_jsx_runtime.jsx("span", { className: S.sessionInlineCost, children: view.costLabel })
				]
			});
		}

		/** Zero-chrome `conversation.input.right` metadata before the host model selector. */
		function CurrentSessionInline({ sessionId, session, modelDirectory, useProjection = () => void 0, request = fetchJson, t }) {
			...
			return react_jsx_runtime.jsx(CurrentSessionInlineView, {
		 ...
		 * the zero-chrome current-session summary before the host model selector.
		...
		function apply(ctx) {
			...
			ctx.inject(["slots", "modelDirectories"], (scope) => {
				...
				scope.slots.inject("conversation.input.right", () => scope.slots.register({
					...
					id: "usage-stats-current-session-inline",
					locale: NS,
					order: 10,
		...
		exports.CurrentSessionInline = CurrentSessionInline;
		exports.CurrentSessionInlineView = CurrentSessionInlineView;
		// v0.3.0 compatibility aliases: the public display key and client exports
		// retain their historical Pill names while rendering inline metadata.
		exports.CurrentSessionPill = CurrentSessionInline;
		exports.CurrentSessionPillView = CurrentSessionInlineView;
