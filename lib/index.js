...
import { resolveCurrentTariff } from "./pricing.js";
...
async function handleProviders(ctx, config, accounts, req, res) {
	if (rejectForeignCaller(req, res)) return;
	try {
		const [views, configured] = await Promise.all([
			accounts.providerViews(),
			configuredProviders(ctx)
		]);
		const byId = new Map(configured.map((provider) => [provider.id, provider]));
		const providers = views.map((view) => {
			const provider = byId.get(view.id);
			if (provider === void 0) return view;
			const identity = resolveProviderIdentity(provider, config);
			const pricingWindow = resolveCurrentTariff({ identity, timestamp: Date.now(), currency: "USD" });
			return pricingWindow === null ? view : { ...view, pricingWindow };
		});
...
async function apply(ctx, rawConfig = {}, deps = {}) {
	...
	ctx.effect(() => ctx.webServer.register({
		kind: "exact",
		path: PROVIDERS_PATH,
		handler: (req, res) => handleProviders(ctx, config, accounts, req, res)
