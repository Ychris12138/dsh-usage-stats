...
async function handleSubscriptions(ctx, accounts, req, res) {
	if (rejectForeignCaller(req, res)) return;
	try {
		const subscriptions = (await accounts.subscriptionAccounts()).filter(Boolean);
...
function apply(ctx, rawConfig = {}) {
	...
	if (config.backgroundRefresh) ctx.effect(() => startBackgroundRefresh(ctx, accounts, { intervalMs: config.refreshIntervalMs }), "usage-stats: background account refresh");
}

export { apply, inject, name, USAGE_PATH, PROVIDERS_PATH, BALANCE_PATH, SUBSCRIPTIONS_PATH, ACCOUNT_PATH, configuredProviders, totalTokens, zeroBuckets };
