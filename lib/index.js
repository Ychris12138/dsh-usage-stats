...
function freshCache(pricingFingerprintValue, previous = null, transitionAt = Date.now()) {
	...
	const cache = {
		...
		sessions: {},
		// Stored sessions this backend refused to read, keyed by the revision that
		// failed. DSH 0.1.7 refuses historical logs whose event descriptors it no
		// longer supports, and each attempt costs a full decode; the entry is
		// dropped as soon as the log's revision changes.
		failures: fingerprintChanged ? { ...previous.failures } : {}
	};
...
function restoredCache(parsed, pricingFingerprintValue) {
	...
	const cache = {
		...
		sessions,
		failures: parseFailures(parsed.failures)
	};
	runtimeOf(cache);
	return cache;
}

/** Restore the unreadable-session memo (lenient: any malformed entry is dropped). */
function parseFailures(raw) {
	const failures = {};
	if (raw === null || typeof raw !== "object" || Array.isArray(raw)) return failures;
	for (const [id, revision] of Object.entries(raw)) {
		if (id !== "" && typeof revision === "string" && revision !== "") failures[id] = revision;
	}
	return failures;
}
...
/**
 * Render one aggregate plus its per-session rows and budgets. Pure over the
 * cache: it never folds, mutates, or saves, so a reader may call it while a
 * collection is in flight.
 */
function renderCollection(cache, byDay, billingByDay, config) {
 ...
 * timeout, so a UI read during one serves the last aggregate instead; the scan
 * publishes the fresh one when it finishes and the next read picks it up.
 */
function renderCachedCollection(ctx, cache, config) {
	const runtime = runtimeOf(cache);
	if (runtime.aggregate !== null) {
		return renderCollection(cache, runtime.aggregate.byDay, runtime.aggregate.billingByDay, config);
	}
	// The in-flight scan owns `runtime.aggregate`, so this response must not race
	// its rebuild by publishing an aggregate of its own.
	...
	return renderCollection(cache, byDay, billingByDay, config);
}
		...
		if (scanPersisted && persistence !== void 0) {
			...
			if (snapshots !== null) {
				...
				for (const { header: meta, revision } of snapshots) {
					...
					if (stateChanged) markCacheChanged(cache);
					// A log this backend already refused at this exact revision is not
					// retried: the attempt costs a full decode and cannot succeed until
					// the log changes.
					if (typeof revision === "string" && cache.failures[id] === revision) continue;
				...
				const worker = async () => {
					for (;;) {
						...
						try {
							...
							if (typeof revision === "string" && cache.failures[id] !== void 0) {
								delete cache.failures[id];
								markCacheChanged(cache);
							}
						} catch (error) {
							if (typeof revision === "string") {
								cache.failures[id] = revision;
								markCacheChanged(cache);
							}
				...
				if (failures.length > 0) {
					// One summary per scan: an unreadable session is reported once, then
					// stays silent until its log changes.
					ctx.logger.warn(`usage-stats: ${failures.length} stored session(s) could not be read and stay excluded until their log changes (first: "${failures[0].id}": ${String(failures[0].error)})`);
				}
