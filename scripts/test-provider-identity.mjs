...

{
	...
	assert.equal(hostname.accountAdapter, null, "OrcaRouter has no verified balance adapter");
	...
	assert.equal(resolveAccountSpec(provider("orcarouter", "https://api.orcarouter.ai/v1")).adapter, null, "OrcaRouter must not start account polling");
}
