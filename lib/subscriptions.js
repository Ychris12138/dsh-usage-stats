...
async function collectKimi(credentials, deps) {
	...
	try {
		...
		return { id: "kimi", displayName: "Kimi For Coding", mode: "subscription", status: parsed.windows.length > 0 ? "ok" : "unavailable", ...parsed };
	} catch (error) {
...
function parseMiniMax(body) {
	...
	const general = remains.find((entry) => String(entry?.model_name ?? entry?.modelName ?? "").toLowerCase() === "general") ?? remains[0];
...
async function collectMiniMax(credentials, deps) {
	...
	const urls = configuredUrl === null ? [
		...
		`${MINIMAX_HOSTS[region]}/v1/token_plan/remains`
	] : [configuredUrl];
	try {
		let body = null;
		...
		const windows = parseMiniMax(body);
		return { id: "minimax", displayName: "MiniMax Coding Plan", mode: "subscription", status: windows.length > 0 ? "ok" : "unavailable", plan: "MiniMax Coding Plan", region, windows };
	} catch (error) {
