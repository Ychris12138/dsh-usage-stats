...
function defaultAdapter(providerId) {
...
export function validateAccountConfig(raw = {}) {
	...
	const refreshIntervalMs = raw.refreshIntervalMs === void 0 ? DEFAULT_REFRESH_MS : numberOrNull(raw.refreshIntervalMs);
	if (refreshIntervalMs === null || refreshIntervalMs < 60000) throw new Error("refreshIntervalMs must be at least 60000");
	return {
		monitors: normalized,
...
export function resolveAccountSpec(provider, config = { monitors: {} }) {
	...
	const adapter = monitor.adapter ?? defaultAdapter(provider.id);
	...
	return {
		...
		adapter,
		mode,
		apiKeyRef: monitor.credentialRef ?? provider.apiKeyEnv,
...
async function quotaPerUnit(spec, deps) {
	try {
		...
		/* Older deployments may not expose status; use the historical default. */
	}
...
async function queryNewApi(spec, credentials, credential, deps, now) {
	...
	const balance = {
		...
		currency: "USD",
		unlimited,
		expiresAt: toIso(body.data.expires_at)
	};
	return {
		...baseSnapshot(spec, "ok", now),
...
export async function queryAccount(spec, credentials, deps = {}) {
	...
	try {
		if (spec.adapter === "declarative") return await queryDeclarative(spec, credentials, deps, now);
		...
		if (schemeOfAdapter(spec.adapter) !== null) return await queryBuiltInBalance(spec, credential, deps, now);
		if (spec.adapter === "general") return await queryGeneral(spec, credential, deps, now);
		if (spec.adapter === "new-api") return await queryNewApi(spec, credentials, credential, deps, now);
...
export function createAccountService({ credentials, getProviders, config = { monitors: {} }, deps = {} }) {
	...
	async function subscriptionAccounts() {
		const all = await specs();
		return Promise.all(all.filter((spec) => spec.mode === "subscription").map((spec) => get(spec.id)));
	}
