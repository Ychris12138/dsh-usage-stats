	...
	factory: (require) => {
		...
		function buildProviderChoices(providers, subscriptions) {
			const choices = Array.isArray(providers) ? providers.map((provider) => ({
				...provider,
				accountMode: provider.accountMode ?? (subscriptionIdFor(provider.id) === null ? "balance" : "subscription"),
				subscriptionId: provider.subscriptionId ?? subscriptionIdFor(provider.id)
			})) : [];
